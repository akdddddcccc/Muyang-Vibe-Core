  ...
  const payload = await requestDeepSeekJson([
    {
      ...
      content: [
        ...
        "Create a fresh schedule from task semantics. Ignore any existing child startDay, endDay or lane values; they are not user locks.",
        "Infer relative effort, prerequisite order and safe parallelism from each title and note instead of dividing time evenly or preserving the previous layout.",
        "Treat parent and child notes as customization constraints. Do not give every long-running task the entire parent range by default; assign purposeful phases and visibly different durations.",
        "Tasks on the same lane must not overlap. Independent tasks may overlap on different lanes. Use dependsOn only for real finish-to-start prerequisites.",
        "Every dependency must end before the dependent task starts. A simple one-to-one continuation should use the same lane as its prerequisite; a task joining multiple prerequisite branches may use a new lane.",
